package Adapter; /**
 * @author zhp
 * @date 2022-08-21 22:38
 */

/**适配器
 * 1) 适配器模式(Adapter.Adapter Pattern)将某个类的接口转换成客户端期望的另一个接口表示，主的目的是兼容性，让原本
 ...
 *  1.类适配器模式
 *      基本介绍：
 *      Adapter.Adapter 类，通过继承 src 类，实现 dst 类接口，完成 src->dst 的适配
 *      注意事项和细节：
 *      1) Java 是单继承机制，所以类适配器需要继承 src 类这一点算是一个缺点, 因为这要求 dst 必须是接口，有一定局
 *      限性;
 *      2) src 类的方法在 Adapter.Adapter 中都会暴露出来，也增加了使用的成本。
 *      3) 由于其继承了 src 类，所以它可以根据需求重写 src 类的方法，使得 Adapter.Adapter 的灵活性增强了
 *
 *2.对象适配器模式
 *      基本介绍：
 *      1) 基本思路和类的适配器模式相同，只是将 Adapter.Adapter 类作修改，不是继承 src 类，而是持有 src 类的实例，以解决
